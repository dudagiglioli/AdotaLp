...
public class SituacaoAnimalRpositoryImpl implements SituacaoAnimalQuery{
    ...
    private Predicate[] criarrestricoes(SituacaoAnimalFilter situacaosanimalfilter, CriteriaBuilder builder, Root<SituacaoAnimal> root){

        List<Predicate> predicates = new ArrayList<>();

        if (!StringUtils.isEmpty(situacaosanimalfilter.getNomeanimal())){
            predicates.add(builder.like(builder.lower(root.get("nomeanimal")),
                    "%" + situacaosanimalfilter.getNomeanimal().toLowerCase() + "%"));
        }

        if (!StringUtils.isEmpty(situacaosanimalfilter.getDescricao())){
            predicates.add(builder.like(builder.lower(root.get("descricao")),
                    "%" + situacaosanimalfilter.getDescricao().toLowerCase() + "%"));
        }
